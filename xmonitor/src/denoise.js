...
const PERSONAL_INTRO_RE = /\bintroduc(?:e|ing|es)?\b[^.!?\n]{0,40}\b(my ?sel[fh]|himself|herself|themselves|each other|my (?:girlfriend|boyfriend|cat|dog|friend|new)|you all|y'all|u all)\b/i;
const NEWS_STYLE_RE = /^\s*(?:🚨\s*)?(?:break(?:ing)?|just in|it'?s official|official statement|confirmed[::]|reports?:|update[::])\b/i;
const SPORTS_RE = /\b(?:\d+(?:st|nd|rd|th)\s+(?:HR|home run)|hits? (?:his|her|a) (?:\d+|home run)|grand slam|walk-?off|goal (?:against|of the season)|match point|knock(?:ed)? ?out (?:in|at) round)\b/i;
// Product signals.
const LEADING_LAUNCH_RE = /^\s*(?:🚀|🎉|✨|\p{Extended_Pictographic}\s)?\s*(?:we'?re|we are|just|introducing|proudly?|finally|today (?:we))?[\s\S]{0,20}?(introduc\w*|launch\w*|unveil\w*|announc\w*|shipp?ed|going live|now live)/iu;
const PRODUCT_VOCAB_RE = /\b(app|api|beta|saas|tool|platform|extension|dashboard|marketplace|open[- ]source|waitlist|sign ?up|download|pricing|free tier|v\d(?:\.\d)?|mcp|cli|sdk|ios|android|compare|analytics|automat\w+|agent\w*|editor|library|framework|template|boilerplate)\b/i;
// "Introducing the RetroTINK-6X CE:" / "Launching GemEx - ..." — mixed-case
// naming (ALLCAPS shouting doesn't count) followed by a : / — / - separator.
const NAMED_TARGET_RE = /\b(?:introduc\w*|launch\w*|unveil\w*)\s+(?:the\s+|our\s+|my\s+|an?\s+)?[A-Z][a-z][\w .-]{0,48}[:—-]\s/;
export function evaluateLaunch(input) {
    ...
    if (!evidence)
        reasons.push("no-product-evidence");
